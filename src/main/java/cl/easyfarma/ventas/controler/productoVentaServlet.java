 ...
 * @author nlast
 */
public class productoVentaServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        processRequest(request, response);
         try {
            ...
            List<ProductoVenta> productos = productoVentaDAO.obtenerProductosPorNumeroDocumento(numeroDocumento);
            ...
            System.err.println("JSON "+jsonProductos);
            response.setContentType("application/json");
            response.setCharacterEncoding("UTF-8");
            response.getWriter().write(jsonProductos);
        } catch (Exception e) {
            e.printStackTrace();
            response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Error al obtener los productos de venta");
        }
    }
